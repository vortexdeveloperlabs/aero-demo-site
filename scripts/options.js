...
searchDropdown.addEventListener("change", (event) => localStorage.setItem("search", event.target.value));
...
const setbgColor = (event) => {
	...
	if (root instanceof HTMLElement) root.style.setProperty("--bg", event.target.value);
...
const setfgColor = (event) => {
	...
	if (root instanceof HTMLElement) root.style.setProperty("--fg", event.target.value);
...
transportDropdown.value = localStorage.getItem("transport-path") ?? "/libcurl/index.mjs";
transportDropdown.addEventListener("change", (event) => localStorage.setItem("transport-path", event.target.value));

const wispUrl = document.getElementById("wisp-url");
wispUrl.value = localStorage.getItem("wisp-url") ?? `${location.protocol === "https:" ? "wss" : "ws"}://${location.host}/wisp/`;
...
bareUrl.value = localStorage.getItem("bare-url") ?? `${location.protocol}//${location.host}/bare/`;
